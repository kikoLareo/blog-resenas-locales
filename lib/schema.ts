...
export function reviewJsonLd(review: Review, venue: Venue): ReviewJsonLd {
  // Calcular rating promedio (escala 0-10)
  const avgRating = (
    review.ratings.food + 
...
export function articleJsonLd(
  ...
  const baseUrl = SITE_CONFIG.url;
  const url = `${baseUrl}/${venue.city.slug.current}/${venue.slug.current}/${review.slug.current}`;
  ...
  return {
    ...
    image: images || review.gallery?.map(img => img.asset.url).slice(0, 3) || [],
    datePublished: review.publishedAt,
    dateModified: review.publishedAt,
    author: {
      '@type': 'Person',
      name: review.author,
    },
