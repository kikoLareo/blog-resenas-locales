...
export function getOptimizedImageUrl(
  ...
  if (config.auto === 'format') {
    ...
    imageBuilder = imageBuilder.auto('format');
    imageBuilder = imageBuilder.quality(Math.min(85, config.quality || 85));
  }
 ...
 */
export function isValidImage(image: unknown): image is SanityImage {
  return (
    !!image &&
    typeof image === 'object' &&
    'asset' in image &&
    !!(image as { asset?: unknown }).asset &&
    typeof (image as { asset: { _id?: unknown } }).asset._id === 'string' &&
    typeof (image as { asset: { url?: unknown } }).asset.url === 'string'
  );
}
